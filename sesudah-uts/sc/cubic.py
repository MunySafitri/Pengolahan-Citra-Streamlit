...
def cubic_interpolation(image, scale_factor):
...
def main():
    st.title("Image Cubic Interpolation App")
    ...
    if uploaded_image is not None:
        ...
        if st.button("Interpolate Image"):
            interpolated_image = cubic_interpolation(image, scale_factor)
            st.image(interpolated_image, caption=f"Cubic Interpolated Image (Scale Factor: {scale_factor})", use_column_width=True)
